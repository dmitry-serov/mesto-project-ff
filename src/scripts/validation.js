...
const showInputError = (formElement, inputElement, errorMessage, config) => {
  const errorElement = formElement.querySelector(`.${inputElement.name}-input-error`); // выбираем элемент для текста ошибки
...
const toggleButtonState = (inputList, buttonElement, config) => {
  if (hasInvalidInput(inputList)) { // если есть невалидный инпут
